...
std::vector<GenericType*> CreateWrapperFuncGenericParams(FuncBase* rawFunc, const std::string& funcIdentifier,
...
void WrapMutFunc::CreateMutFuncWrapper(FuncBase* rawFunc, CustomTypeDef& curDef, ClassType& srcClassTy)
{
    ...
    if (isImported) {
        funcBase = builder.CreateImportedVarOrFunc<ImportedFunc>(
            wrapperFuncTy, funcIdentifier, "", "", pkgName, wrapperGenericParams);
    } else {
        funcBase = builder.CreateFunc(INVALID_LOCATION, wrapperFuncTy, funcIdentifier, "", "", pkgName,
            wrapperGenericParams);
    }
